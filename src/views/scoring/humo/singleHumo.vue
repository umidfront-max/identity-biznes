...
import { HumoResponseModel, singleHumo_API, getHumoResponse_DEFAULT, HumoDetailsModel } from '/@/services/scoring/humo'
...
const _item = ref<HumoResponseModel>(getHumoResponse_DEFAULT())
...
async function loadItems() {
  _loading.value = true
  const [error, response] = await singleHumo_API(route.query.id)
...
<template>
  <div>
    ...
    <div v-else class="card-wrap default-table-detail-container">
      <el-page-header :content="'Humo ' + card_UTIL(_item.request.number as number)" @back="goBack" />
      <div class="credit-card-container">
        <tiny-credit-card-form :background-image="backgroundImage" :form-data="formData" />
      </div>
      <div>
        <h2>{{ $t('scoring_humo.report') }}</h2>
        <el-table v-if="_item.response.report.length" :data="_item.response.report" style="width: 100%">
          ...
          <el-table-column label="Oy" prop="month" width="200">
            <template #default="{ row }">
              <p class="table-border">{{ formatDate_UTIL(row.month) }}</p>
            </template>
          </el-table-column>
          <el-table-column :label="t('scoring_humo.payment_info')" prop="details" width="400">
            <template #default="{ row }">
              <div v-for="(item, ind) in row.details" :key="ind" class="table-column">
                <p class="table-border">{{ item.description }}</p>
              </div>
              <p class="table-border">{{ $t('scoring_humo.total') }}</p>
            </template>
          </el-table-column>

          <el-table-column :label="t('scoring_humo.term')">
            <template #default="{ row }">
              <div v-for="(item, ind) in row.details" :key="ind" class="table-column">
                <p class="table-border">{{ item.amount.toLocaleString() }}</p>
              </div>
              <p class="table-border">
                {{ row.details.reduce((acc: number, current: HumoDetailsModel) => acc + current.amount, 0)?.toLocaleString() }}
              </p>
            </template>
          </el-table-column>
        </el-table>
        <p v-else style="font-size: 18px; color: red; text-align: center">{{ $t('scoring_humo.no_payments') }}</p>
      </div>
    </div>
  </div>
</template>
