...
import { HumoResponseModel, getHumo_API } from '/@/services/scoring/humo'
...
const { t } = useI18n()
const _items = ref<HumoResponseModel[]>([])
...
const tableColumns = [
  {
    label: t('scoring_humo.columns.status'),
    prop: 'status',
    sortable: true,
    checked: true,
    iconUsing: (row: HumoResponseModel) => {
  ...
  {
    label: t('scoring_humo.columns.card_number'),
    prop: 'number',
    checked: true,
    displayUsing: (row: HumoResponseModel) => card_UTIL(row.request.number as number),
  },
  {
    label: t('scoring_humo.columns.card_expiry_date'),
    prop: 'passport_series',
    checked: true,
    displayUsing: (row: HumoResponseModel) =>
      `${row.request.expiry?.toString().substring(2, 4)}/${row.request.expiry?.toString().substring(2, 0)}`,
  },
  {
    label: t('scoring_humo.columns.created_at'),
    prop: 'created_at',
    sortable: true,
    displayUsing: (row: HumoResponseModel) => formatDate_UTIL(new Date(row.created_at * 1000), false),
  },
  {
    label: t('scoring_humo.columns.updated_at'),
    prop: 'updated_at',
    displayUsing: (row: HumoResponseModel) => formatDate_UTIL(new Date(row.created_at * 1000), false),
  },
]
...
async function loadItems(item: QueryDataModel) {
  _loading.value = true
  const [error, response] = await getHumo_API(item)
...
const navigateToSingleView = (item: HumoResponseModel) => {
  if (item.id)
    router.push({
      path: '/scoring/single-humo',
      query: {
        id: item.id,
      },
    })
}
const deleteBranchs = (items: HumoResponseModel | HumoResponseModel[]) => {
  ...
  <div>
    ...
    >
      <template #table-standalone-actions="{ selectedRows }">
        <el-button
          v-if="hasAccess(PERMISSION_ENUM.scoring_humo_view)"
          :icon="AddIcon"
          size="large"
          type="primary"
          @click="$router.push('/scoring/create-humo')"
        >
          {{ $t('buttons.add') }}
        </el-button>
        <el-button
          v-if="selectedRows.length > 0 && hasAccess(PERMISSION_ENUM.scoring_humo_delete)"
          :icon="DeleteIcon"
          size="large"
          type="danger"
      ...
      <template #table-inline-actions="{ row }">
        <el-button
          v-if="hasAccess(PERMISSION_ENUM.scoring_humo_delete)"
          :icon="DeleteIcon"
          type="danger"
