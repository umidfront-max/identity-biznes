...
import { getUzcardRequest_DEFAULT, UzcardRequestModel, createUzcard_API } from '/@/services/scoring/uzcard'
...
const rules = ref<FormRules>({
  expiry: [
    { required: true, trigger: 'blur', message: t('scoring_uzcard.messages.fill_field') },
    { min: 5, trigger: 'blur', message: t('scoring_uzcard.messages.fill_field') },
  ],
  number: [
    { required: true, trigger: 'blur', message: t('scoring_uzcard.messages.fill_field') },
    { min: 19, trigger: 'blur', message: t('scoring_uzcard.messages.card_number_is_16_characters') },
  ],
})
...
const _cardInfo = ref<UzcardRequestModel>(getUzcardRequest_DEFAULT())
...
async function createUzcard() {
  formRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      const [error] = await createUzcard_API(data)
      if (error) {
        handleValidate(error.errors)
        return
      }
      $baseMessage(t('scoring_uzcard.messages.card_added_successfully'), 'success', 'hey')
      router.push('/scoring/uzcard')
    }
  })
}
...
<template>
  <div class="default-table-detail-container">
    <el-page-header content="Uzcard" @back="goBack" />
    <div class="credit-card-container">
      ...
      <el-card class="card-item">
        <el-form ref="formRef" label-position="top" :model="_cardInfo" :rules="rules">
          <el-form-item ref="formItemRef" :label="t('scoring_uzcard.enter_card_number')" prop="number">
            <el-input
          ...
          <el-form-item ref="formItemRef1" :label="t('scoring_uzcard.enter_card_expiry_date')" prop="expiry">
            <el-input v-model="_cardInfo.expiry" v-mask="'##/##'" maxlength="5" size="large" />
          </el-form-item>
          <el-form-item style="width: 100%">
            <el-button size="large" style="width: 100%" type="primary" @click="createUzcard">{{ $t('buttons.create') }}</el-button>
          </el-form-item>
        </el-form>
      </el-card>
    </div>
